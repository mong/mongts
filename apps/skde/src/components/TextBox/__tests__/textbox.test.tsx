...
test("Standard render", async () => {
  ...
  const { container } = render(
    <TextBox atlas={{ lang: "en" } as Atlas}>{markdownText}</TextBox>,
  );
...
test("Longer text", async () => {
  ...
  const { container } = render(
    <TextBox atlas={{ lang: "nb" } as Atlas}>{markdownText}</TextBox>,
  );
  expect(container).toMatchSnapshot();
});
...
test("Text with image", async () => {
  ...
  const { container } = render(
    <TextBox atlas={{ lang: "nn" } as Atlas}>{markdownText}</TextBox>,
  );
  expect(container).toMatchSnapshot();
});
