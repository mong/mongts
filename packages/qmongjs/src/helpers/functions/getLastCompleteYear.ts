 ...
 *
 * @param {Date | undefined | null} delivery_latest_affirm - An array of Indicator objects containing the delivery_latest_affirm date.
...
export function getLastCompleteYear(
  delivery_latest_affirm: Date | undefined | null,
  ...
  const delivery_latest_affirm_year = delivery_latest_affirm
    ? new Date(
        new Date(delivery_latest_affirm).setDate(
          new Date(delivery_latest_affirm).getDate() + 2,
        ),
      ).getFullYear() - 1
    : undefined;
